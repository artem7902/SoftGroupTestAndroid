...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pref=this.getApplicationContext().getSharedPreferences("EmailPass", 0); // 0 - for private mode
        if(pref.getString("email",  null)!=null){
            Intent loginInt = new Intent(this, AccountActivity.class);
            loginInt.putExtra("email", pref.getString("email",  null));
    ...
    public void sing_inClick(View view ){
        ...
        final EditText password = (EditText) findViewById(R.id.password_input);
            if(email.getText().toString().isEmpty() || password.getText().toString().isEmpty() || !db.ValidateUser(email.getText().toString(), password.getText().toString()))
